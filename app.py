...
def load_data_from_sheets(sheet_key):
    ...
    try:
        ...
        if not all(col in df.columns for col in required_columns):
            ...
            return None
        # THAY ĐỔI 1: Chuyển đổi sang datetime nhưng giữ lại toàn bộ thông tin thời gian
...
def analyze_scores_for_chatbot(scores_df):
    ...
    latest_date = latest_scores['Date'].strftime('%d-%m-%Y %H:%M:%S') # Giữ nguyên định dạng chi tiết
...
if scores_df is not None and not scores_df.empty:
    with st.expander("Xem biểu đồ điểm nguy hiểm của bệnh", expanded=True):
        # THAY ĐỔI 2: Lấy phần ngày (.date()) để làm giá trị cho slider
        min_date_slider = scores_df['Date'].min().date()
        max_date_slider = scores_df['Date'].max().date()
        ...
        # THAY ĐỔI 3: Lọc dataframe dựa trên phần ngày của cột 'Date'
        filtered_df = scores_df[
            (scores_df['Date'].dt.date >= start_date) & 
            (scores_df['Date'].dt.date <= end_date)
        ]
        ...
        if not filtered_df.empty:
            ...
            # THAY ĐỔI 4: Sử dụng cột 'Date' (kiểu datetime) cho trục X để hiển thị chi tiết
            line_chart = alt.Chart(scores_melted).mark_line().encode(
                x=alt.X('Date:T', title='Thời gian cập nhật'), # ':T' để Altair hiểu đây là dữ liệu thời gian
                y=alt.Y('Điểm nguy hiểm', scale=alt.Scale(domain=[0, 10])),
                color='Tên bệnh',
                tooltip=['Date:T', 'Tên bệnh', 'Điểm nguy hiểm'] # ':T' để tooltip hiển thị đúng định dạng
            ).interactive()
